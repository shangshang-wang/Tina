#!/bin/bash
# Local machine launch helper: waits for specific local eval queues and defaults
# to this host's GPU IDs 5,6,7.
set -euo pipefail
...
ADAPTER_TYPE="grpo_open_rs3_plan50_ablation"
CKPT="checkpoint-250"
...
GPU_IDLE_POLL_SECONDS="${GPU_IDLE_POLL_SECONDS:-60}"

read -r -a SEED_LIST <<< "${SEEDS:-0 1 2 3 4}"
read -r -a TASKS <<< "${EVAL_TASKS:-aime24 aime25 amc23 math_500 minerva gpqa:diamond}"
read -r -a GPU_LIST <<< "${GPUS:-5 6 7}"

QUEUE_DIR="${PROJECT_DIR}/.cache/eval_queue/${ADAPTER_TYPE}_${CKPT}_seeds_${SEED_LIST[*]// /_}"
CLAIMS_DIR="${QUEUE_DIR}/claims"
LOG_DIR="${LOGGING_DIR}/gpu_queue_${ADAPTER_TYPE}_${CKPT}_wait_5_7_$(date +%Y%m%d_%H%M%S)"
...
echo "GPU_IDLE_MEMORY_MB: ${GPU_IDLE_MEMORY_MB}"
echo "GPU_IDLE_POLL_SECONDS: ${GPU_IDLE_POLL_SECONDS}"
...
wait_for_existing_eval_queues() {
    local monitor_log="${LOG_DIR}/wait_existing_eval_queues.log"
    while true; do
        mapfile -t existing_pids < <(
            pgrep -af "scripts/eval/eval_open_rs3_baseline_gpu_queue_multi_seeds.sh" \
                | awk -v self="$$" '$1 != self {print $1}'
        )
        if [[ "${#existing_pids[@]}" -eq 0 ]]; then
            echo "[monitor] no existing baseline eval queue found $(date)" | tee -a "${monitor_log}"
            return 0
        fi
        echo "[monitor] waiting for existing baseline eval queue pids: ${existing_pids[*]} $(date)" | tee -a "${monitor_log}"
...
wait_for_gpu_idle() {
    ...
    while ! gpu_is_idle "${gpu}"; do
        local used_memory
        used_memory="$(nvidia-smi -i "${gpu}" --query-gpu=memory.used --format=csv,noheader,nounits | tr -d ' ')"
        echo "[gpu ${gpu}] waiting for idle: memory.used=${used_memory}MiB $(date)" | tee -a "${worker_log}"
        sleep "${GPU_IDLE_POLL_SECONDS}"
    done
    echo "[gpu ${gpu}] idle, starting queued work $(date)" | tee -a "${worker_log}"
}

if [[ ! -d "${MODEL_PATH}" ]]; then
    echo "Merged model not found. Merging ${ADAPTER_TYPE}/${CKPT} first."
    (
        export CUDA_VISIBLE_DEVICES="${MERGE_GPU:-0}"
...
task_output_file() {
    ...
    local seed="$2"
    local filename
    filename="${seed}-${TEMPERATURE}-${TOP_P}-${task}-${MAX_NEW_TOKENS}"
    if [[ "${MAX_MODEL_LENGTH}" != "${MAX_NEW_TOKENS}" ]]; then
        filename+="-${MAX_MODEL_LENGTH}"
    fi
    printf "%s/%s/%s/%s/%s/%s.json" \
        "${OUTPUT_DIR}" "${task}" "${seed}" "${MODEL_NAME_FOR_OUTPUT}" "${MODEL_FOLDER}" "${filename}"
}
...
worker() {
    ...
    while true; do
        ...
        wait_for_gpu_idle "${gpu}" "${worker_log}"
