#!/bin/bash
# Local machine launch helper: GPU queue tuned for this host's 8x24GB setup and
# local offline Hugging Face/cache directories.
set -euo pipefail
...
MODEL_FOLDER="_home_wangls_Tina_ckpts_models_DeepSeek-R1-Distill-Qwen-1.5B_base"
...
# - top_p: 0.95
# - no system prompt
# This script improves utilization by running one independent eval worker per GPU.
TEMPERATURE=0.6
TOP_P=0.95
...
QUEUE_DIR="${PROJECT_DIR}/.cache/eval_queue/deepseek_distill_qwen_1_5b_seeds0_4"
CLAIMS_DIR="${QUEUE_DIR}/claims"
LOG_DIR="${LOGGING_DIR}/gpu_queue_deepseek_distill_qwen_1_5b_$(date +%Y%m%d_%H%M%S)"
...
task_output_file() {
    ...
    printf "%s/%s/%s/%s/%s/%s-%s-%s-%s-%s-%s.json" \
        ...
        "${seed}" "${TEMPERATURE}" "${TOP_P}" "${task}" "${MAX_NEW_TOKENS}" "${MAX_MODEL_LENGTH}"
}
...
worker() {
    ...
    while true; do
        ...
        for seed_candidate in "${SEED_LIST[@]}"; do
            for task_candidate in "${TASKS[@]}"; do
                ...
                if mkdir "${CLAIMS_DIR}/${claim_name}" 2>/dev/null; then
                    printf "%s\n" "gpu=${gpu}" "pid=$$" "host=$(hostname)" "time=$(date -Iseconds)" "task=${task_candidate}" "seed=${seed_candidate}" > "${CLAIMS_DIR}/${claim_name}/claim.txt"
                    claimed="${claim_name}"
                    task="${task_candidate}"
                    seed="${seed_candidate}"
                    break 2
        ...
        (
            ...
            export TMPDIR="${PROJECT_DIR}/.cache/ray_tmp/gpu${gpu}"
            export RAY_TMPDIR="${TMPDIR}"
            mkdir -p "${TMPDIR}"
