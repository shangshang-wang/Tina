...
GPU_INDEX="${GPU_INDEX:?Set GPU_INDEX, e.g. GPU_INDEX=1}"
SEED="${SEED:?Set SEED, e.g. SEED=3}"
...
LOG_FILE="${LOGGING_DIR}/eval_${PT_CONFIG_NAME}_${CKPT}_gpu${GPU_INDEX}_seed${SEED}_$(date +%Y%m%d_%H%M%S).log"

{
  echo "START eval ${PT_CONFIG_NAME} ${CKPT} seed ${SEED} on GPU ${GPU_INDEX} $(date)"
  ...
  echo "END eval ${PT_CONFIG_NAME} ${CKPT} seed ${SEED} on GPU ${GPU_INDEX} $(date)"
} 2>&1 | tee -a "${LOG_FILE}"
